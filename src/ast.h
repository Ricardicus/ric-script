...
#define DEF_NEW_CONTEXT()        \
  ...
  locals_stack_t *varLocals;     \
  int interactive;               \
  class_t *classCtx = NULL;      \
  void *syncCtx = NULL;          \
  hashtable_t *classDecs = NULL; \
  hashtable_t *funcDecs = NULL;  \
  hashtable_t *varDecs = NULL;
#define DEF_NEW_CONTEXT_STATIC()    \
  static context_full_t exeCtx;     \
  static int32_t r0, r1, r2, ax;    \
  static double f0, f1, f2;         \
  static void *sp, *sb, *hp, *hb;   \
  static void *st, *ed;             \
  static size_t sc;                 \
  static int depth;                 \
  static locals_stack_t *varLocals; \
  static int interactive;           \
  static class_t *classCtx;         \
  static void *syncCtx;             \
  static hashtable_t *classDecs;    \
  static hashtable_t *funcDecs;     \
...
#define PROVIDE_CONTEXT_ARGS_OLD()                                                          \
  int32_t *r0, int32_t *r1, int32_t *r2, int32_t *ax, double *f0, double *f1, double *f2,   \
      void *sp, void *sb, void *hp, void *hb, void **st, void **ed, size_t *sc, int *depth, \
      locals_stack_t *varLocals, int *interactive, class_t *classCtx, void *syncCtx
#define EXPRESSION_PARAMS() \
  void *stmt, void *next, PROVIDE_CONTEXT_ARGS(), argsList_t *args, hashtable_t *argVals
...
#define SETUP_STACK(sp, sb, sz, sc)                        \
  do {                                                     \
    intptr_t p;                                            \
